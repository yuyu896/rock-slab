...
async function handleImportFile(event: Event) {
  ...
  try {
    const { data } = await importAssetStocks(file)
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="import-step">
          ...
          <p class="import-step-desc">模板列：分公司、资产编号、资产类目、物品分类、资产名称、数量、规格、警戒线；同一分公司同一资产编号只能一行（已存在会报错，请编辑该行）</p>
        ...
        <div class="import-step">
          ...
          <label class="import-upload-area" :class="{ 'upload-loading': importing }">
            ...
            <template v-else>
              ...
              <span class="import-upload-hint">支持 .xlsx / .xls 格式，每次不超过 200 行</span>
            </template>
          </label>
        </div>
