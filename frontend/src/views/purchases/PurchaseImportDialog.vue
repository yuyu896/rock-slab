...
import { ref } from 'vue'
import { importTransfers } from '@/api/transfers'
...
function handleDownloadTemplate() {
  generateTransferTemplate('采购入库导入模板', 'purchase')
}
...
async function confirmImport() {
  ...
  try {
    const { data } = await importTransfers(importFile.value)
  ...
  <div v-if="visible" class="modal-overlay" @click.self="emit('close')">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="import-row">
          <div class="import-row-info">
            ...
            <span class="import-row-desc">按格式填写采购记录后上传</span>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn-confirm" @click="confirmImport" :disabled="importing || !importFile">{{ importing ? '导入中...' : '确认导入' }}</button>
      </div>
    </div>
  </div>
</template>
...
.btn-confirm { padding: 6px 18px; border-radius: 8px; border: none; background: var(--color-primary); color: #fff; cursor: pointer; font-size: 13px; }
