...
- [x] 1.7 处置扣列快照（部署前拦下的缺陷修复）：TransferLine 加「处置扣列」（0026 AddField + 0027 存量回填=旧规则在用×数量）；apply 生效时写实况快照；对账重放与 line_effective_plan 离线回退统一读快照——修复处置单重放零扣导致的对账差异（曾误判为本地手测残差）
