...
# 生产为两层反向代理（root-nginx-1:443 → rock-slab-nginx:8080 → Gunicorn），
# 两层都追加 X-Forwarded-For：DRF 取倒数第 2 个地址 = 真实客户端 IP。
# 若误配为 1，全站用户会共享同一个登录限流桶（曾在线上实测发生）。
REST_FRAMEWORK['NUM_PROXIES'] = int(os.environ.get('NUM_PROXIES', '2'))  # noqa: F405
