...
def _run(*args):
    out = StringIO()
    call_command('prod_data_fix_nb33_2', *args, stdout=out)
    return out.getvalue()
...
class TestNb33PointDelete:
    def test_point_delete_syncs_line_and_stock(self, nb33, second_branch):
        item = _item('NB33B-I1')
        d1 = _multi_instance_doc(nb33, item, 6, 'CG20250227-004')
        d2 = _multi_instance_doc(nb33, item, 13, 'CG20250205-004')
        # 他分公司同品目基准
        _multi_instance_doc(second_branch, item, 5, 'CG20250404-099')

        from unittest.mock import patch
        from apps.assets.management.commands.prod_data_fix_nb33_2 import TARGET_NUMBERS
        ...
        with patch('apps.assets.management.commands.prod_data_fix_nb33_2.TARGET_NUMBERS', targets):
            out = _run('--apply')
    ...
    def test_missing_target_skips(self, nb33):
        _multi_instance_doc(nb33, _item('NB33B-I4'), 1, 'CG20250611-003')
